...
class IONOSService {
  ...
  initialize() {
    if (!this.client && config.aiProvider === 'ionos') {
      this.client = new OpenAI({
        baseURL: 'https://openai.inference.de-txl.ionos.com/v1',
        apiKey: config.ionos.apiKey
      });
    }
  }
  ...
  async analyzeDocument(content, existingTags = [], existingCorrespondentList = [], existingDocumentTypesList = [], id, customPrompt = null, options = {}) {
    ...
    try {
      ...
      if (!this.client) {
        throw new Error('IONOS client not initialized');
      }
      ...
      let systemPrompt = '';
      let promptTags = '';
      const model = process.env.IONOS_MODEL || config.ionos.model;
      ...
      console.error('Failed to analyze document with IONOS:', error);
      return {
  ...
  async _validateAndTruncateExternalApiData(apiData, maxTokens = 500) {
    ...
    const dataTokens = await calculateTokens(dataString, process.env.IONOS_MODEL || config.ionos.model);

    if (dataTokens > maxTokens) {
      console.warn(`[WARNING] External API data (${dataTokens} tokens) exceeds limit (${maxTokens}), truncating`);
      return await truncateToTokenLimit(dataString, maxTokens, process.env.IONOS_MODEL || config.ionos.model);
    }
  ...
  async analyzePlayground(content, prompt) {
    ...
    try {
      ...
      if (!this.client) {
        throw new Error('IONOS client not initialized - missing API key');
      }
      ...
      console.error('Failed to analyze playground document with IONOS:', error);
      return {
        document: { tags: [], correspondent: null },
        metrics: null,
        error: error.message
  ...
  async generateText(prompt) {
    try {
      ...
      console.error('Error generating text with IONOS:', error);
      throw error;
    }
  }

  async checkStatus() {
    // send test request to IONOS API and respond with 'ok' or 'error'
    try {
      this.initialize();
      ...
      const response = await this.client.chat.completions.create({
        model: process.env.IONOS_MODEL || config.ionos.model,
        messages: [
          {
            role: "user",
            content: "Test"
      ...
      return { status: 'ok', model: process.env.IONOS_MODEL || config.ionos.model };
    } catch (error) {
      console.error('Error checking IONOS status:', error);
